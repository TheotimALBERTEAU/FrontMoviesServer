...
export class EpisodePage implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private watchAnime: WatchAnime,
  ...
  private loadData(slug: string) {
    this.watchAnime.getEpisode(slug, parseInt(this.seasonNum), parseInt(this.episodeNum)).subscribe({
      next: (data: any) => {
        this.episode = data.data;
        this.totalSeasons = this.episode.totalSeasons;

        this.watchAnime.getSeason(slug, parseInt(this.seasonNum)).subscribe((res: any) => {
  ...
  changeEpisode(delta: number) {
    ...
    this.router.navigate([`/animes/${this.episode.slug}/${this.seasonNum}-${nextEp}`]);
    this.cdr.detectChanges();
  }
  ...
  changeSeason(num: number) {
    ...
    if (slug) {
      this.watchAnime.getSeason(slug, num).subscribe((res: any) => {
        this.seasonEpisodes = res.data.episodes;
        this.cdr.detectChanges();
      });
    }
